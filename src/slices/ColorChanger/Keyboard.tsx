...
// Custom props for keyboard customization
type KeyboardProps = JSX.IntrinsicElements["group"] & {
  keycapMaterial?: THREE.MeshStandardMaterial;
  keycapMaterial2?: THREE.MeshStandardMaterial;
  extrudeColor?: string;
  frameColor?: string;
};
...
export const Keyboard = forwardRef<KeyboardRef, KeyboardProps>(
  (
    { keycapMaterial, keycapMaterial2, extrudeColor, frameColor, ...props },
    ref,
  ) => {
    ...
    // Use custom materials if provided, otherwise use defaults from GLTF
    const keycapMat = keycapMaterial || materials["Keycaps 1"];
    const keycapMat2 = keycapMaterial2 || materials["Keycaps 2"];

    // Create extrude material from color if provided
    const extrudeMat = useMemo(() => {
      if (extrudeColor) {
    ...
    }, [extrudeColor, materials.Symbols]);

    // Create frame material from color if provided
    const frameMat = useMemo(() => {
      ...
      return materials["Main Frame"];
    }, [frameColor, materials["Main Frame"]]);
