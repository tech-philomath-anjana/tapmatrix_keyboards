...
export function Scene() {
  ...
  // Ensure the hero keyboard uses the Goodwell UV texture explicitly so
  // it doesn't fall back to any other theme or a stale build asset.
  const keycapTexture = useTexture("/goodwell_uv.png");
  keycapTexture.flipY = false;
  // Set color space to match how other textures are handled in the project
  // (keeps colors accurate in PBR materials).
  // @ts-ignore - drei's texture typing can be slightly different here
  keycapTexture.colorSpace = THREE.SRGBColorSpace;

  const heroKeycapMaterial = new THREE.MeshStandardMaterial({
    roughness: 0.7,
    map: keycapTexture,
  });
      ...
      <group scale={scalingFactor}>
        <group ref={keyboardGroupRef}>
          <Keyboard
            ...
            keycapMaterial={heroKeycapMaterial}
          />
        </group>
