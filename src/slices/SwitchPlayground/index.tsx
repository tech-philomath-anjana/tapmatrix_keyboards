...
const SwitchPlayground: FC<SwitchPlaygroundProps> = ({ slice }) => {
  return (
    <Bounded
      ...
      className="relative"
      innerClassName="fex flex-col justify-center"
    >
      <FadeIn>
        <h2
...
const SharedCanvas = ({ color }: SharedCanvasProps) => {
  ...
  return (
    <div className="group relative min-h-96 overflow-hidden rounded-3xl select-none">
      ...
      <button
        onClick={handleSound}
        className="font-bold-slanted absolute bottom-0 left-0 z-10 flex items-center gap-3 p-6 text-3xl text-white uppercase focus:ring-2 focus:ring-white focus:outline-none"
      >
        {name} <LuVolume2 />
      </button>
      {/* Canvas */}
